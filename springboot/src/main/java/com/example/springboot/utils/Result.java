...
import static com.example.springboot.enums.StatusCodeEnum.FAIL;
import static com.example.springboot.enums.StatusCodeEnum.SUCCESS;

@Data
public class Result<T> extends PageInfo<EquipInfo> {
    ...
    private static <T> Result<T> buildResult(Boolean flag, T data, Integer code, String message) {
        Result<T> r = new com.example.springboot.utils.Result<T>();
        r.setFlag(flag);
        r.setData(data);
        r.setCode(code);
        r.setMsg(message);
