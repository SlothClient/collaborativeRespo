...
const getBigNums = async () => {
  const response = await request.post('/api/getBigNums');
  try {
...
const getEquipPlans = async () => {
  const response = await request.post('/api/getEquipPlans');
  try {
    if (response.data.status) {
      /**
...
const getPlanTypes = async () => {
  const response = await request.post('/api/getPlanTypes');
  try {
    if (response.data.status) {
      // 将返回的list数据转换成图表所需格式
...
const getWorkerRank = async () => {
  const response = await request.post('/api/getWorkerRank');
  try {
    if (response.data.status) {
      const workerIds = [];
      const workerNames = [];
...
const getEquipType = async () => {
  const res = await request.post('/api/getEquipType');
  console.log(res);
